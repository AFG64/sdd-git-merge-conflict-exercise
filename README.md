# Merge Conflict Exercise: Online Checkout
...
repo per group, not one per person. Its git history is what gets graded, so
it must contain every feature branch and the merge-conflict resolution
commit. Keep the fork **public** (the default when forking a public repo)
so it can be graded it without needing collaborator access.
**Each student submits the link to their own feature's pull request on
Blackboard** (see step 10), not just the repo link.

## The setup

`store.py` has one function, `calculate_total`, with a single TODO comment.
The function already accepts one optional argument per feature
(`apply_discount`, `apply_tax`, `apply_shipping`), each defaulting to
`False`. Your job is to add the rule's logic at the TODO line, gated behind
your feature's argument (e.g. `if apply_discount: ...`). Every feature is
implemented at that same TODO line on purpose. That's what guarantees a real
conflict when you merge two of them together, even though each rule only
runs when its own argument is `True`.
...
There are two test suites, one per group size: `test_store_2person.py` and
`test_store_3person.py`. Use whichever one matches your group throughout
this exercise; ignore the other one.
...
Verify with the `test_all_features_combined` test in your group's suite.
...
environment automatically, e.g. for a 2-person group:

```bash
uv run pytest test_store_2person.py
uv run pytest test_store_2person.py::test_tax_feature_alone   # run a single test
```

or for a 3-person group:

```bash
uv run pytest test_store_3person.py
uv run pytest test_store_3person.py::test_tax_feature_alone   # run a single test
```
...
Sanity-check your own feature in isolation, using your group's test suite
(`test_store_2person.py` or `test_store_3person.py`):

```bash
uv run pytest test_store_<2person|3person>.py::test_<feature>_feature_alone
```
...
git add store.py
git commit
uv run pytest test_store_<2person|3person>.py::test_all_features_combined
...
On `main`, all tests in your group's suite should now pass:

```bash
uv run pytest test_store_<2person|3person>.py
```

`test_all_features_combined` passing confirms every teammate's feature
survived the conflict resolution intact.

**Every student submits the link to the pull request for the feature they
built on Blackboard, individually.** This is the PR link from step 6, not
the repo's URL. Submitting the PR link shows your specific
commits and diff directly, even after the PR has been merged into `main`.
Each teammate's link will be different, since each points to their own
feature branch's PR, but all PRs must live in the same Repo Owner's fork.
The fork's overall git history should show: each feature branch's commit(s),
and a merge commit where the conflict in `store.py` was resolved.
